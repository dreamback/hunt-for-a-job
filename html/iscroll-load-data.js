...
function IScrollLoadData(wrapperEl,contentEl,dropTopAction,dropBottomAction,limit){
	...
	this.pullDownEl=this.wrapper.querySelector('#pullDown');
	this.pullUpEl=this.wrapper.querySelector('#pullUp');
...
IScrollLoadData.prototype.pullDownAction=function(){
	...
	if(this.dropTopAction){
		this.dropTopAction(function(out){
			if(out){
				if(out == 'nodata'){
					new Popup('toast',null,"success",null,"已经全部加载完毕");
				}else if(out == 'fail'){
					new Popup('toast',null,"error",null,"网络出问题啦,稍后再试吧!");
				}
			}
			self.refresh('top');
...
IScrollLoadData.prototype.pullUpAction=function(){
	...
	this.myScroll.scrollToElement(lis[lis.length-1], null, null, true);
};
