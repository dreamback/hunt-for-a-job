...
(function ($) {   
    ...
    $.fn.datePicker = function (options) {    
        return this.each(function(e) { 
            ...
            function init(){//初始化函数
                initVal=that.val()||opts.defaultValue.replace(/-0(\d)/g,function($0,$1){return '-'+$1});
                if(!$('#datePlugin').size()){
            ...
            function renderDom(){
                var mainHtml=' <div class="d-date-box"><p class="d-date-info"><span class="d-day-info"></span><span class="d-return-info"></span></p></div>';
            ...
            function createYear(){
                ...
                yearDom.find('li').eq(initY).addClass('current');
                yearScroll.on('scrollEnd', function(event) {
                    var yIndex=this.currentPage.pageY;
                    initY=yIndex+1;
                    if(isLeap(parseInt(yearDom.find('li').removeClass('current').eq(initY).addClass('current').data('num')))){
                        opts.monthDay[1]=29;
                    }
                    else{
                        opts.monthDay[1]=28;
            ...
            function createMonth(){
                ...
                monthDom.find('li').eq(initM).addClass('current');
                ...
                monthScroll.on('scrollEnd', function(event) {
                    ...
                    showTxt();
                    monthDom.find('li').removeClass('current').eq(initM).addClass('current');
                });
            }
            function createDay(dayNum){
                ...
                dayDom.find('li').eq(initD).addClass('current');
                dayScroll.on('scrollEnd', function(event) {
                    initD=this.currentPage.pageY+1;
                    showTxt();
                    dayDom.find('li').removeClass('current').eq(initD).addClass('current');
                });
            }
            function createHour(){
